...
module LocalizationSimplified
  ...
  # from RUtils plugin
  # FIXME: in some cases, somewhere will cache the time.wday, and make the localized weekday messed up.
...
module ActiveRecord
  ...
  class Base
    def self.human_attribute_name attrib
      ...
      human_attribute_names[attrib.to_sym] || attrib.humanize
    end

    def self.human_name
      (self.const_defined? "ModelName") ? self::ModelName : self.name.humanize
    end
      
    # to solve the problem of db rejecting Date type values
    private 
    def quote_value(value, column = nil)
      value = value.to_s(:db) if value.kind_of?(Date)
...
module ActionView
  module Helpers
    ...
    # <%=_ english_words %> would be translated to localized words which definded in LocalizationSimplified::Translations
    # LocalizationSimplified::Translations can be defined in each lang_xxx.rb in plugin,
    # as well as a separated file in $RAILS_ROOT/config, such as $RAILS_ROOT/config/lang.rb.
    # If you define Translation const in config folder, the definitions can take effect without restarting server in development enviroment.
    module TranslationHelper
        # Usage: <%=_ "Create" %>
        #        if LocalizationSimplified::Translations["Create"] is defined, a translated string will be replaced.
        def _ string_to_localize, *args
          translations = LocalizationSimplified.const_defined?("Translations") ? LocalizationSimplified::Translations : {}
          translated = translations[string_to_localize] || string_to_localize
     ...
     module ActiveRecordHelper
       ...
       def error_messages_for(*params)
         ...
         # if defined modelname, use this localized information
         klass = objects[0].class
         options[:object_name] = klass::ModelName if klass.const_defined? "ModelName"
...
class Array
  ...
  def to_sentence(options = {})
    ...
    # added two option for asian usage, :comma to replace "," and :remove_space (true or false) to remove the space after connector
    options.assert_valid_keys(:connector, :skip_last_comma, :comma, :remove_space)
    options.reverse_merge! :connector => 'and', :skip_last_comma => false, :comma => ', ', :remove_space => false

    space = options[:remove_space] ? '' : ' '
    connector = space + options[:connector] + space
    case length
     when 0
       ""
     when 1
     ...
     when 2
       ...
       "#{self[0...-1].join(options[:comma])}#{options[:skip_last_comma] ? '' : ','}#{connector}#{self[-1]}"
    end
  end
...
# for active_scaffold l10n
def as_ (string_to_localize, *args)
  translations = LocalizationSimplified.const_defined?("Translations") ? LocalizationSimplified::Translations : {}
  translated = translations[string_to_localize] || string_to_localize
  args.empty? ? translated : (sprintf translated, *args)
end

# fixed conflict between plugin calendar_date_select and l10n
CalendarDateSelect.format[:date] = "%m/%d/%Y" if Object.const_defined?("CalendarDateSelect")

# override active_scaffold config for defining human names
=begin
Sample human name definition in model:
class Team << ActiveRecord::Base
  ModelName = "Grnz"
  ColumnNames = {
    :name => "anzr",
    :created_at => "Perngrq ng"
  }
end
=end
module ActiveScaffold
  module ClassMethods
    def active_scaffold_with_human_name(model_id = nil, &block)
      active_scaffold_without_human_name model_id, &block

      # assign model names
      config = self.active_scaffold_config
      klass = config.model

      config.label = klass::ModelName if klass.const_defined?('ModelName')
      klass::ColumnNames.each do |column, human_name|
        config.columns[column].label = human_name if config.columns[column]
      end if klass.const_defined?('ColumnNames')
    end

    alias_method_chain :active_scaffold, :human_name
  end
end
